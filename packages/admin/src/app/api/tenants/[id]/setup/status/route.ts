...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const res = await fetch(`https://${inst.railway_domain}/setup/api/status`, {
      cache: 'no-store',
      headers: { 'Authorization': basicAuthHeader(inst.setup_password) },
    });
    ...
    return jsonError(e?.message || 'Failed to fetch setup status');
  }
}
