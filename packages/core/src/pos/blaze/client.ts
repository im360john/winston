...
export interface BlazeRequestOptions {
  method?: 'GET' | 'POST' | 'PUT' | 'PATCH';
  query?: Record<string, string | number | boolean | undefined>;
  body?: unknown;
}

export interface BlazeClientConfig extends PosConnectionConfig {}
...
export class BlazeClient {
  ...
  constructor(private readonly config: BlazeClientConfig) {
  ...
  private request(options: BlazeRequestOptions, path: string): Promise<unknown> {
    return new Promise((resolve, reject) => {
      ...
      const req = transport.request(reqOptions, (res) => {
        ...
        res.on('end', () => {
          ...
          if (status === 401 || status === 403) {
            return reject(new PosAdapterError(`Blaze auth error: HTTP ${status}`, 'AUTH', false));
          }

          if (status === 429) {
            return reject(new PosAdapterError('Blaze rate limit exceeded', 'RATE_LIMIT', true));
          }

          if (status === 404) {
            return reject(new PosAdapterError(`Blaze resource not found: ${path}`, 'NOT_FOUND', false));
          }

          if (status >= 500) {
            return reject(new PosAdapterError(`Blaze server error: HTTP ${status}`, 'NETWORK', true));
          }

          if (status < 200 || status >= 300) {
            return reject(new PosAdapterError(`Blaze unexpected status: HTTP ${status}`, 'UNKNOWN', false));
          }

          try {
            resolve(data ? JSON.parse(data) : {});
          } catch (e) {
            reject(new PosAdapterError('Blaze response is not valid JSON', 'PARSE', false, e));
          }
        });
      });
      ...
      req.setTimeout(this.timeoutMs, () => {
        ...
        reject(new PosAdapterError(
          `Blaze request timed out after ${this.timeoutMs}ms`,
          'NETWORK',
          true,
        ));
      });
      ...
      req.on('error', (err: Error) => {
        reject(new PosAdapterError(`Blaze network error: ${err.message}`, 'NETWORK', true, err));
      });
