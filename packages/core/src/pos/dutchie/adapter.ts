...
import { DutchieClient, DutchieClientConfig } from './client';
import {
  normalizeProduct,
  normalizeInventory,
  normalizeSale,
...
export class DutchieAdapter extends BasePosAdapter {
  readonly posType = 'dutchie';

  private readonly client: DutchieClient;
  private readonly tenantId: string;

  constructor(config: DutchieClientConfig & { tenantId: string }) {
    super();
    this.tenantId = config.tenantId;
    this.client = new DutchieClient(config);
  }
  ...
  async connect(): Promise<void> {
    try {
      ...
      throw new PosAdapterError(
        `Dutchie connection failed: ${(err as Error).message}`,
        'NETWORK',
        true,
        err,
      );
  ...
  async getProducts(filter: ProductFilter = {}): Promise<Page<Product>> {
    ...
    const raw = await this.client.getProducts(params) as DutchieListResponse;
  ...
  async getInventory(filter: InventoryFilter = {}): Promise<Page<InventoryRecord>> {
    ...
    const raw = await this.client.getInventory(params) as DutchieListResponse;
    const items = extractItems(raw);
    return buildPage(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  async getSales(filter: SaleFilter = {}): Promise<Page<SaleTransaction>> {
    ...
    const raw = await this.client.getSales(params) as DutchieListResponse;
    const items = extractItems(raw);
    return buildPage(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  async getCustomers(filter: CustomerFilter = {}): Promise<Page<Customer>> {
    ...
    const raw = await this.client.getCustomers(params) as DutchieListResponse;
    const items = extractItems(raw);
    return buildPage(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  private assertConnected(): void {
    if (!this._connected) {
      throw new PosAdapterError(
        'DutchieAdapter.connect() must be called before querying data',
        'UNKNOWN',
        false,
      );
    }
...
function extractItems(raw: DutchieListResponse): unknown[] {
  return raw.data ?? raw.items ?? [];
}

function buildPage<T>(
  items: T[],
  raw: DutchieListResponse,
